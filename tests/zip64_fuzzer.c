...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    if(!mz_zip_reader_init_mem(&zip_reader, data, size, flags)) return 0;
    if(!mz_zip_writer_init_heap_v2(&zip_writer, 0, size, flags | MZ_ZIP_FLAG_WRITE_ZIP64)) return 0;
    ...
    for(i=0; i < files; i++)
    {
        ...
        if(mz_zip_reader_is_file_a_directory(&zip_reader, i)) continue;

        mz_zip_validate_file(&zip_reader, i, MZ_ZIP_FLAG_VALIDATE_HEADERS_ONLY);

        if(mz_zip_reader_is_file_encrypted(&zip_reader, i)) continue;

        mz_zip_clear_last_error(&zip_reader);

        mz_uint ret = mz_zip_reader_get_filename(&zip_reader, i, filename, filename_max);

        if(mz_zip_get_last_error(&zip_reader)) continue;
        ...
        if (!mz_zip_writer_add_from_zip_reader(&zip_writer, &zip_reader, i)) continue;
    }

    if (!mz_zip_writer_finalize_archive(&zip_writer)) return 1;
